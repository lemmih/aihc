...
-- | Install a package and write the verbose and timing messages to the
-- given handle. The tests give a file handle here. They must not redirect
-- the process stdout instead: the test runner writes its own progress to
-- stdout from other threads, and that output would land in the capture.
